...
export async function authenticateUser(
  ...
  try {
    ...
    if (userData.password.startsWith("$2")) {
      ...
      try {
        console.log(`[MIGRATE] Hashing password for ${email}`);
        const { error: updateError } = await supabase 
        .from("employees")
        .update({ password: hashed, password_changed: true, updated_at: new Date().toISOString()})
        .ilike("email", email.trim());
         
        if (updateError) {
          console.error(`[MIGRATE] Failed to update ${email}:`, updateError);
        } else {
          console.log(`[MIGRATE] Successfully hashed password for ${email}`);
        }

      } catch(err) {
        console.error(`[MIGRATE] Error hashing/updating for ${email}:`, err);
      }

    }  
    
    console.log(`[TEST LOG] New hashed password: ${password}`);
    console.log(`[TEST LOG] UserData password: ${userData.password}`);
    if(!password_matches) {
      return { success: false }
    }
